...
TEST(TracingAckHandler, Ack) {
  ...
  ScopedLog log;
  TracingAckHandler tested(std::move(mock), "test-id");
  tested.ack();
...
TEST(TracingAckHandler, AckSuccess) {
  auto mock = std::make_unique<MockExactlyOnceAckHandlerImpl>();
...
TEST(TracingAckHandler, AckEmpty) {
  auto mock = std::make_unique<MockExactlyOnceAckHandlerImpl>();
  EXPECT_CALL(*mock, ack)
      .WillOnce(
          Return(ByMove(make_ready_future(PermissionDeniedError("uh-oh")))));
  ScopedLog log;
  TracingAckHandler tested(std::move(mock));
  tested.ack();
  EXPECT_THAT(log.ExtractLines(), Not(Contains(HasSubstr(" ack()"))));
}

TEST(TracingAckHandler, Nack) {
  auto mock = std::make_unique<MockExactlyOnceAckHandlerImpl>();
...
TEST(TracingAckHandler, NackSuccess) {
  auto mock = std::make_unique<MockExactlyOnceAckHandlerImpl>();
  EXPECT_CALL(*mock, nack)
...
TEST(TracingAckHandler, NackEmpty) {
  auto mock = std::make_unique<MockExactlyOnceAckHandlerImpl>();
  EXPECT_CALL(*mock, nack)
      .WillOnce(
          Return(ByMove(make_ready_future(PermissionDeniedError("uh-oh")))));
...
TEST(TracingAckHandler, DeliveryAttempt) {
  auto mock = std::make_unique<MockExactlyOnceAckHandlerImpl>();
