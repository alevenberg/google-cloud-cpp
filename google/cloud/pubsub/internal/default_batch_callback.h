...
class DefaultBatchCallback : public BatchCallback {
 public:
  ...
  void callback(MessageCallback::MessageAndHandler m) override {
    message_callback_->callback(std::move(m));
  };
  void callback(MessageCallback::ReceivedMessage m) override {
    message_callback_->callback(std::move(m));
  };
