  ...
  *request.add_ack_ids() = ack_id;
  ...
  return stub_
      ->AsyncModifyAckDeadline(cq_, std::make_shared<grpc::ClientContext>(),
                               request)
      .then([calllback = callback_, ack_id](auto f) {
        calllback->EndNackMessage(ack_id);
        return f;
      });
}
                 ...
                 [this, callback = callback_, ack_ids](auto const& request) {
                   return stub_
                       ->AsyncModifyAckDeadline(
                           ...
                           request)
                       .then([callback = callback, ack_ids](auto f) {
...
void StreamingSubscriptionBatchSource::OnRetryFailure(Status status) {
  ...
  callback_->operator()(BatchCallback::StreamingPullResponse{std::move(status), absl::nullopt});
}
...
void StreamingSubscriptionBatchSource::OnRead(
  ...
  if (response && stream_state_ == StreamState::kActive && !shutdown_) {
    ...
    lk.unlock();
    callback_->operator()(
        BatchCallback::StreamingPullResponse{*std::move(response), absl::nullopt});
