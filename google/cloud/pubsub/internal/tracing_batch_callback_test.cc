...
std::shared_ptr<BatchCallback> MakeTestBatchCallback(
  ...
  return MakeTracingBatchCallback(std::move(mock), TestSubscription());
}
...
BatchCallback::StreamingPullResponse MakeResponse(int n) {
  ...
  for (int i = 0; i < n; i++) {
    auto m = r.add_message();
    m.add_message_ids("id-" + n);
    m.add_ack_ids("ack-id-" + n);
  }
  return response;
}
...
TEST(TracingBatchCallback, StartAndEndMessage) {
  ...
  auto batch_sink = MakeTestBatchCallback(std::move(mock));

  batch_sink->callback(MakeResponse(1));
  batch_sink->EndMessage("ack-id-1", "test-event");

  auto spans = span_catcher->GetSpans();
  EXPECT_THAT(
      spans, Contains(AllOf(SpanHasInstrumentationScope(), SpanKindIsInternal(),
                            SpanNamed("test-sub subscribe"),
                            SpanHasEvents("test-event"))));
}
