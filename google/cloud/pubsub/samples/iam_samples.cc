...
void AutoRun(std::vector<std::string> const& argv) {
  ...
  request.set_name(
      google::cloud::pubsub::Subscription(project_id, subscription).FullName());
  request.set_topic(google::cloud::pubsub::Topic(project_id, topic).FullName());
  auto subscription =
      subscription_admin_client.CreateSubscription(request).value();
