 ...
 *
 * When applications pull a message from Cloud Pub/Sub messages the response
 * includes both the `pubsub::Message` and its associated
 * `pubsub::PullAckHandler`. Actions on a `pubsub::PullAckHandler` always affect
 * the same message received in the callback. Applications cannot create
 * standalone handlers (except in unit tests via mocks).
 *
...
class PullAckHandler {
 public:
  ...
  PullAckHandler& operator=(PullAckHandler&&) = default;
  ~PullAckHandler();
   ...
   */
  explicit PullAckHandler(std::unique_ptr<Impl> impl)
      : impl_(std::move(impl)) {}
...
#endif  // GOOGLE_CLOUD_CPP_GOOGLE_CLOUD_PUBSUB_PULL_ACK_HANDLER_H
