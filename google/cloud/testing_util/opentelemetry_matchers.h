...
std::string ToString(opentelemetry::trace::SpanContext span_context);
...
MATCHER_P(LinkHasSpanContext, context, "has context" + google::cloud::testing_util::ToString(context)) {
  auto const& actual = arg.GetSpanContext();
  *result_listener << "has context: " << google::cloud::testing_util::ToString(actual) ;
